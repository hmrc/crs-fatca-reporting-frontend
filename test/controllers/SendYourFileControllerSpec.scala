    ...
    "getStatus" - {
      ...
      "must return OK and return journey recovery url when status is RejectedSDES" in {
        ...
        running(application) {
          ...
          status(result) mustEqual OK
          contentAsJson(result).toString mustEqual "{\"url\":\"/report-for-crs-and-fatca/report/there-is-a-problem\"}"
        }
      }
      ...
      "must return OK and return file failed checks url when status is Rejected" in {
        ...
        running(application) {
          ...
          status(result) mustEqual OK
          contentAsJson(result).toString mustEqual "{\"url\":\"/report-for-crs-and-fatca/report/problem/rules-errors\"}"
        }
      }
