  ...
  "FileValidationController" - {
    ...
    "must redirect to invalid XML page if XML validation fails" in {
      ...
      when(mockValidationConnector.sendForValidation(any())(any(), any()))
        .thenReturn(Future.successful(Left(SchemaValidationErrors(ValidationErrors(errors, None)))))
      when(mockSessionRepository.set(any())).thenReturn(Future.successful(true))
      ...
      status(result) mustBe SEE_OTHER
      redirectLocation(result).value mustEqual routes.RulesErrorController.onPageLoad().url
    ...
    "must redirect to invalid FIID page if FIID does not match" in {
