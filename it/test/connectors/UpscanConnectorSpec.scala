...
class UpscanConnectorSpec extends AnyFreeSpec with ISpecBase {
  ...
  "getUploadDetails" - {
    "should return an UploadSessionDetails" - {
      "when a valid successful response is returned" in {
        ...
        stubGetResponse("/crs-fatca-reporting/upscan/details/12345", OK, Json.toJson(body).toString())
    ...
    "should return None" - {
      "when an invalid response is returned" in {
        stubGetResponse("/crs-fatca-reporting/upscan/details/12345", OK, Json.obj().toString())
  ...
  "getUploadStatus" - {
    "should return an UploadStatus for a valid UploadId" - {
      "when an UploadedSuccessfully response is returned" in {
        ...
        stubGetResponse("/crs-fatca-reporting/upscan/status/12345", OK, body)
    ...
    "should return None" - {
      "when an invalid response is returned" in {
        stubGetResponse("/crs-fatca-reporting/upscan/status/12345", OK, Json.obj().toString())
