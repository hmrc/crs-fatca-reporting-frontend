...
class ChangeFileController @Inject() (
  override val messagesApi: MessagesApi,
  sessionRepository: SessionRepository,
  identify: IdentifierAction,
  getData: DataRetrievalAction,
  requireData: DataRequiredAction,
  formProvider: ChangeFileFormProvider,
  ...
  def onPageLoad(): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      Ok(view(form, giinOrElectionsProvided(request.userAnswers)))
  }
  ...
  private def giinOrElectionsProvided(ua: UserAnswers) =
    ua.get(RequiredGiinPage).isDefined ||
      ua.get(ReportElectionsPage).exists(identity)
}
